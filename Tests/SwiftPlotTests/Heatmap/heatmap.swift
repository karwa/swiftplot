...
//    hm3.parent?.values = 10..<20
    
//    hm3.minimumSeparation = 8
    
//    var hm3 = "THIS IS SWIFPLOT!!!! Woo let's see what this looks like :)"
//      .plots.barChart(adapter: .keyPath(\.asciiValue!), origin: "0")
//            .stackedWith(-15..<0, adapter: .linear, origin: 0)
    
//                      distanceFunc: { Float(Int($1.asciiValue!) - Int($0.asciiValue!)) })
    hm3.segmentColor = .green
//    hm3.parent?.parent?.minimumSeparation = 4
//    hm3.parent?.parent?.graphOrientation = .horizontal

//    var hm3 = Array(stride(from: Float(0), to: 1, by: 0.005))
//      .plots.barChart(interpolator: .linear, origin: 0, distanceFunc: { $0.distance(to: $1) })
//    var hm3 = "😘 you too".utf8
//      .plots.barChart(origin: 0)
//    hm3.colorMap = .viridis
    ...
    hm2.values = (0..<10).map { row in
      (0..<10).map { col in 0 }
>>>>>>> 064c879... Added horizontal charts, including fixing some bugs in the previous implementation.
    }
    try renderAndVerify(heatmap, size: Size(width: 900, height: 450))
  }
