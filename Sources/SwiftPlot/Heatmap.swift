...
extension Heatmap: HasGraphLayout, Plot {
    ...
    public func layoutData(size: Size, renderer: Renderer) -> (DrawingData, PlotMarkers?) {
        ...
        results.desiredPlotSize = Size(
            width: Float(results.columns) * results.itemSize.width,
    ...
    public func drawData(_ data: DrawingData, size: Size, renderer: Renderer) {
        ...
        for (rowIdx, row) in values.enumerated() {
            for (columnIdx, element) in row.enumerated() {
                let rect = Rect(
                    origin: Point(
                        Float(columnIdx) * data.itemSize.width,
...
where Base.Element: Sequence, HeatmapConstraints.IsFloat<Base.Element.Element>: Any {
...
extension SequencePlots
where Base.Element: Sequence, HeatmapConstraints.IsInteger<Base.Element.Element>: Any {
...
extension SequencePlots where Base: Collection {
    ...
    public func heatmap(
        ...
        while rowStart != base.endIndex {
            guard let rowEnd = base.index(rowStart, offsetBy: width, limitedBy: base.endIndex)
            else {
        ...
        func sliceForRow(_ row: Int, width: Int) -> Base.SubSequence {
            guard
                let start = base.index(
                    base.startIndex, offsetBy: row * width, limitedBy: base.endIndex)
            else {
...
extension SequencePlots
where Base: RandomAccessCollection, HeatmapConstraints.IsFloat<Base.Element>: Any {
...
extension SequencePlots
where Base: RandomAccessCollection, HeatmapConstraints.IsInteger<Base.Element>: Any {
